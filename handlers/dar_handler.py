...
# dosya adi .env den alir TELEGRAM_BOT__NAME
import os
import re
import zipfile
...
TELEGRAM_BOT__NAME = os.getenv("TELEGRAM_BOT__NAME", "xbot")  # .env varsa bot adı, yoksa "xbot"
...
async def dar_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode.upper() == "Z":
        zip_filename = f"{TELEGRAM_BOT__NAME}_{timestamp}.zip"
        try:
    ...
    if len(tree_text) > TELEGRAM_MSG_LIMIT:
        txt_filename = f"{TELEGRAM_BOT__NAME}_{timestamp}.txt"
        try:
